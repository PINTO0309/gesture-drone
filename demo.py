...
plugin.add_cpu_extension("extension/x86-64/libcpu_extension.so")
if args.boost == True:
    model_xml = "extension/IR/FP32/pose-estimation-model-light.xml"
else:
    model_xml = "extension/IR/FP32/pose-estimation-model-heavy.xml"
...
################
#gesture_map = [[255] * 480 for i in range(360)]
gesture_map = np.zeros((camera_height, camera_width), dtype=np.uint8)
gesture_point = queue.Queue()
################
...
try:
    ...
    while True:
        ...
        colw = color_image.shape[1] #480
        colh = color_image.shape[0] #360
        ...
        for part in range(nPoints):
            ...
            #print("Keypoints - {} - {} : {}".format(keypointsMapping[part], part, keypoints))
            keypoints_with_id = []
        ...
        for i in range(nPoints):
            for j in range(len(detected_keypoints[i])):
                ...
                if detected_keypoints[i][j][4] == 4:
                    X = keypointsXY[1]
                    Y = keypointsXY[0]
                    gesture_point.put((X, Y))
                    qsize = gesture_point.qsize()
                    ...
                    if qsize > 20:
                        ...
                        for q in range(qsize):
                            X, Y = gesture_point.get()

                            # A area start judgment
                            if X >= 0 and X <= (camera_width // 2)-1:
                                if len(point_list) > 0:
                            ...
                            if X >= (camera_width // 2) and X <= camera_width:
                                if len(point_list) > 0:
                    ...
                    cv2.circle(frameClone, keypointsXY, 5, colors[i], -1, cv2.LINE_AA)
